...
/// An object used by providers to interact with other providers and the life-cycle
/// of the application
///
/// [read] and [watch], 2 methods that allow a provider to consume other provider
...
abstract class Ref<State extends Object?> {
  ...
  ///
  ///If we do not care about the new value, prefer [invalidate] instead
  ...
  ///
  /// As opposed to [refresh], the refresh is not immediate and is instead delayed
  /// to the next read or next frame.
  ///
  /// Calling [invalidate] multiple times will refresh the provider only once.
  ///
  /// Calling [invalidate] will cause the provider to disposed immediately
  ...
  /// Invalidates the state of the provider, causing it to refresh.
  ///
  /// The refesh is not immediate and is instead delayed to the next read
  /// or next frame
  ///
  /// Calling [invalidateSelf] multiple times will refresh the provider only once
  ///
  /// Calling [invalidateSelf] will cause the provider to be disposes immediately.
  void invalidateSelf();
  ...
  /// _will_ get paused/dispose. It is possible that after the last listener is removed,
  /// a new listener is immedately added.
  ///
  /// [AutoDisposeRef.keepAlive], which ca be combined with [onCancel] for
  /// advanced manipulation on when the provider should get disposed.
  /// [Provider.autoDispose], a modifier which tell a provider that it should
  /// destroy its state when no longer listened to.
  ...
  /// [Provider.autoDispose], a modifier which tell a provider that it should
  /// destroy its state when no longer listened to.
  /// [ProviderContainer.dispose] to destroy all providers asscoiated with a
  /// [ProviderContainer] at once.
  /// [onCancel], a life-cycle for when all listener of a provider are removed.
  void onDispose(void Function() cb);

  /// Read the state associated with a provider, without listening to that provider.
  ///
  /// By calling [read] instead of [watch], this will not cause a provider's state
  /// to be recreated when the provider obtained changes
  ///
  ...
  ///
  /// This method should be your go-to way to make a provider read another provider
  /// even if the value exposed by that other provider never changes.
  ///
  /// ## Use-case example: Sorting a todo-list
  ///
  /// Consider a todo-list application. We may want to implement a sort feature,
  /// to see the uncompleted tofos first.
  ...
  /// Riverpod to immdediately call the listener with the current value.
  ///
...
abstract class AutoDisposeRef<State> extends Ref<State> {
  /// Whether to destroy the state of the provider when all listeners are removed
  /// or not.
  ///
  /// Can be changed at any time, in which case when setting it to `false`,
  /// may destroy the provider state if it currently has no listener.
  ///
  /// Reqeusts for the state of a provider to not be disposed when all the
  ...
  /// to be clsoed for the provider to dispose itself when all listeners are removed.
  ///
